...
icon: Code
description: Managing dependencies with pnpm workspaces.
---

## Overview

This project uses [pnpm](https://pnpm.io/) and its [workspace](https://pnpm.io/workspaces) feature to 
manage dependencies across the monorepo.
This guide explains how to add, remove, and update packages.

## Adding a Dependency

To add a dependency to a specific application or package, use the `pnpm add` command with the `--filter` 
flag.
...
To make one package in the workspace a dependency of another
(e.g., making `@repo/database` a dependency of `apps/api`), you can use the `workspace:*` protocol.
